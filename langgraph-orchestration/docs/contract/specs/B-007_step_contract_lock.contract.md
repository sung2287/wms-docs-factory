...
- **Unauthorized Step Prohibited**: PRD-007에 명시된 8가지 Step(`RepoScan`, `ContextSelect`, `RetrieveMemory`, `PromptAssemble`, `LLMCall`, `SummarizeMemory`, `PersistMemory`, `PersistSession`) 외의 사용을 금지한다.
...
- **Extensions Restriction**: `extensions` 필드는 v1에서 반드시 빈 배열(`[]`)이어야 한다.
