...
- **Scope Integrity Rule (LOCK)**: 
  - 허용되지 않은 `scope` 도메인 값 저장을 즉시 거부한다. 
  - Application layer MUST validate `Decision.scope` against the Approved scope allowlist (v1): ['global', 'runtime', 'wms', 'coding', 'ui'] before persistence.
  - Invalid scope MUST trigger Fail-Fast.
...
- Retrieval MUST use the explicit `currentDomain` state.

## 4. Evidence Retrieval Contract
- Evidence는 독립적으로 탐색 가능하며, Decision 로딩 시 자동으로 동반 로딩되지 않는다 (필요 시 링크 테이블을 통해 명시적 요청).

## 5. Anchor Integrity Contract (LOCK)
- Anchor `targetRef` integrity is Soft.
- If `targetRef` does not resolve to an existing Decision/Evidence, the runtime MUST NOT Fail-Fast and MUST continue execution.

## 6. Immediate Persistence Contract
