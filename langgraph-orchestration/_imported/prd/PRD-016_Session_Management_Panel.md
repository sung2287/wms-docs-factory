...
- **Initialization Inconsistency**: `init` 성공 후에도 `list`에 세션이 노출되지 않는 상태는 **CONTRACT_VIOLATION**으로 간주함.
- **Init Safety**: `init`은 idempotent하며 atomic disk write를 보장해야 한다. 위반 시 **CONTRACT_VIOLATION**.
